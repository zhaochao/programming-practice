...
/*
 * Main printer server thread. Accepts connect requests from
 * clients and spawn additional threads to service requests.
 *
 * LOCKING: none.
 */
...
void * client_thread(void *arg)
{
    ...
    sprintf(res.msg, "requst ID %ld", jobid);
    writen(sockfd, &res, sizeof(struct printresp));
...
void * printer_thread(void *arg)
{
    ...
    for (;;) {
        ...
        *icp++ = TAG_OPERATION_ATTR;
        icp = add_option(icp, TAG_CHARSET, "attribute-charset",
                "utf-8");
        icp = add_option(icp, TAG_NATULANG,
                "attribute-natural-language", "en-us");
        sprintf(str, "http://%s:%d", printer_name, IPP_PORT);
        icp = add_option(icp, TAG_URI, "printer_uri", str);
        icp = add_option(icp, TAG_NAMEWOLANG,
                "requesting-user-name", jp->req.usernm);
        icp = add_option(icp, TAG_NAMEWOLANG,
                "job-name", jp->req.jobnm);
...
/*
 * Read and parse the respone from the printer. Return 1
 * if the requst was successful, and 0 for otherwise.
 *
 * LOCKING: none.
 */
...
out:
    free(bp);
    if (nr < 0) {
        log_msg("jobid %ld: error reading printer respone: %s",
