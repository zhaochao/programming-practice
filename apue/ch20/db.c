#include "apue.h"
#include "apue_db.h"
#include <fcntl.h>  /* open and db_open flags */
#include <stdarg.h>
#include <errno.h>
#include <sys/uio.h>    /* structure iovec */
...
#define SEP ':'     /* seperator char in index record */
...
typedef struct {
    ...
    off_t ptroff;       /* chain ptr of offset pointing to this idx record */
    ...
    COUNT cnt_stor2;    /* store: DB_INSERT, found empty, reuse */
...
/*
 * Open or create database. Same arguments as open(2).
 */
DBHANDLE db_open(const char *pathname, int oflag, ...)
{
    ...
    /*
     * Allocate a DB struct, and the buffers it needs.
     */
    len = strlen(pathname);
...
{
    _db_free((DB *)h);      /* close fds, free buffers and struct */
}
 ...
 * in the index file. We read the index record into db->inxbuf
 * and replace the seperators with null bytes. If all is OK we
...
_db_readidx(DB *db, off_t offset)
{
    ...
    /*
     * Read the ascii chain ptr and the acsii length at
    ...
    *ptr2++ = 0;    /* replace SEP with null */
    ...
    if ((db->datlen = atol(ptr2)) <= 0 || db->datlen > DATLEN_MAX)
        err_dump("_db_readidx: invalid length");
    return(db->ptrval);     /* return offset to next key in chain */
}
...
_db_dodelete(DB *db)
{
    ...
    *ptr = 0;   /* null terminate for _db_writedat */
     ...
     * deleted. Recall the _db_find_and_lock sets db->ptroff to
...
_db_writedat(DB *db, const char *data, off_t offset, int whence)
{
    ...
    if (whence == SEEK_END) /* we're appending, lock entire file */
        if (writew_lock(db->datfd, 0, SEEK_SET, 0) < 0)
            err_dump("_db_writedat: write_lock error");
    ...
    if (whence == SEEK_END)
        if (un_lock(db->datfd, 0, SEEK_END, 0) < 0)
            err_dump("_db_writedat: un_lock error");
}

/*
 * Write an index record. _db_writedat is called before
 * this funtion to set the datoff and datlen fields in the
        ...
        off_t offset, int whence, off_t ptrval)
{
    ...
    if (writev(db->idxfd, &iov[0], 2) != PTR_SZ + IDXLEN_SZ + len)
        err_dump("_db_writeidx: writev error of the index record");
...
doreturn:   /* unlock hash chain locked by _db_find_and_lock */
    if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1) < 0)
        err_dump("db_store: unlock error");
    return(rc);
}
